...
# Get the directory of the current script and then its parent
...
class SchedulerSolver:
    ...
    def _index_availability(self) -> None:
        """Pre-index room availability by room ID for efficient lookup."""
        self._room_availability_by_room.clear()
        for i, avail in enumerate(self.room_availability):
    ...
    def _index_student_availability(self) -> dict[int, list[tuple[int, Availability]]]:
        """Index student availability blocks by course ID."""
        result: dict[int, list[tuple[int, Availability]]] = defaultdict(list)
        for i, avail in enumerate(self.student_availability):
    ...
    def _index_teacher_availability(self) -> dict[int, list[tuple[int, Availability]]]:
        """Index teacher availability blocks by course ID."""
        result: dict[int, list[tuple[int, Availability]]] = defaultdict(list)
        for i, avail in enumerate(self.teacher_availability):
